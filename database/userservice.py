...
def register_user_db(name, phone_number, level):
    ...
    if checker:
        ...
        new_user = User(name=name, phone_number=phone_number, level=level, created_at=datetime.now())
        db.add(new_user)
        db.commit()
        return new_user.id
...
def user_answer_db(user_id, q_id, level, user_answer):
    ...
    if exact_question:
        ...
        db.add(new_answer)
        db.commit()
        return True if corectness else False
    else:
...
def plus_point_user_db(user_id, correct_answers):
    ...
    if checker:
        checker.correct_answer += correct_answers
    else:
        ...
        # Если выдаёт ошибку, то исправим
